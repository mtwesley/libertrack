...
class SGS_Form_ORM extends ORM {
  ...
  public function get_errors($args = array(), $with_params = FALSE) {
    $query = DB::select()
      ->from('errors')
    ...
    foreach ($query->execute() as $result) $errors[$result['field']][] = $with_params
      ? array($result['error'] => unserialize($result['params']))
      : $result['error'];
    return (array) $errors;
  }
  ...
  public function set_error($field, $error, $params = array()) {
    DB::insert('errors', array('form_type', 'form_data_id', 'field', 'error', 'type', 'params'))
      ->values(array(static::$type, $this->id, $field, $error, 'E', serialize($params)))
      ->execute();
  }

  public function get_warnings($args = array(), $with_params = FALSE) {
    $query = DB::select()
      ->from('errors')
      ->where('form_type', '=', static::$type)
      ->and_where('form_data_id', '=', $this->id)
      ->and_where('type', '=', 'W');
    foreach ($args as $key => $value) $query->where($key, 'IN', (array) $value);
    foreach ($query->execute() as $result) $warnings[$result['field']][] = $with_params
      ? array($result['error'] => unserialize($result['params']))
      : $result['error'];
  ...
  public function set_warning($field, $warning, $params = array()) {
    DB::insert('errors', array('form_type', 'form_data_id', 'field', 'error', 'type', 'params'))
      ->values(array(static::$type, $this->id, $field, $warning, 'W', serialize($params)))
      ->execute();
  }
