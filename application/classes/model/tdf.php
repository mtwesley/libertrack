...
//        return SGS::volumify(($this->diameter / 100), $this->length);
  ...
  public function parse_data($data)
  {
    foreach ($data as $key => $value) switch ($key) {
      ...
      case 'barcode':
        $this->$key = SGS::lookup_barcode(SGS::barcodify($value), array('F', 'L', 'P')); break;
      
      case 'tree_barcode':
        $this->$key = SGS::lookup_barcode(SGS::barcodify($value), array('T', 'P')); break;
      
      case 'stump_barcode':
        $this->$key = SGS::lookup_barcode(SGS::barcodify($value), array('S', 'P')); break;
  ...
  public function find_duplicates($values, $errors) {
    ...
    $query = DB::select('id')
      ...
      ->and_where('bottom_min', 'BETWEEN', SGS::variance_range(SGS::floatify($values['bottom_min']), SGS::accuracy(self::$type, 'is_matching_diameter')))
