      ...
      switch ($document_type) {
        case 'EXP':
          ...
          $ids = array_filter(DB::select('specs_data.id','barcodes.barcode')
            ...
            ->having(DB::expr('coalesce(remove_from_array(array_agg(distinct "barcode_activity"."activity"::text)), \'{}\')'), '@>', DB::expr("array[/*'D',*/'X']"))
            ->and_having(DB::expr('NOT coalesce(remove_from_array(array_agg(distinct "barcode_activity"."activity"::text)), \'{}\')'), '&&', DB::expr("array['E', 'O', 'H', 'Y', 'A', 'L', 'Z']"))

            ->and_having_open()
              ->or_having(DB::expr('remove_from_array(array_agg(distinct "exp_documents"."id"::text))'), '=', NULL)
              ->or_having(DB::expr('remove_from_array(array_agg(distinct "exp_documents"."id"::text))'), '=', '{}')
            ->and_having_close()
            ->and_having(DB::expr('coalesce(remove_from_array(array_agg(distinct "invoices_paid"."id"::text)), \'{}\')'), '@>', DB::expr('coalesce(remove_from_array(array_agg(distinct "invoices"."id"::text)), \'{}\')'))
        ...
        case 'SPECS':
          ...
          $ids = array_filter(DB::select('specs_data.id','barcodes.barcode')
            ...
            ->having(DB::expr('NOT coalesce(remove_from_array(array_agg(distinct "barcode_activity"."activity"::text)), \'{}\')'), '&&', DB::expr("array['E','O','H','Y','A','L','Z']"))
            ->and_having(DB::expr('NOT coalesce(remove_from_array(array_agg(distinct "parent_barcode_activity"."activity"::text)), \'{}\')'), '&&', DB::expr("array['D','E','O','H','Y','A','L','S','Z']"))
            ->and_having(DB::expr('NOT coalesce(remove_from_array(array_agg(distinct "children_barcode_activity"."activity"::text)), \'{}\')'), '&&', DB::expr("array['D','E','O','H','Y','A','L','S','Z']"))

            ->and_having_open()
                ->or_having(DB::expr('remove_from_array(array_agg(distinct "documents"."id"::text))'), '=', NULL)
                ->or_having(DB::expr('remove_from_array(array_agg(distinct "documents"."id"::text))'), '=', '{}')
            ->and_having_close()
            ->and_having_open()
                ->or_having(DB::expr('remove_from_array(array_agg(distinct "related_documents"."id"::text))'), '=', NULL)
                ->or_having(DB::expr('remove_from_array(array_agg(distinct "related_documents"."id"::text))'), '=', '{}')
                ->or_having(DB::expr('coalesce(remove_from_array(array_agg(distinct "barcode_activity"."activity"::text)), \'{}\')'), '@>', DB::expr("array['S']"))
            ->and_having_close()

            ->and_having_open()
                ->or_having(DB::expr('remove_from_array(array_agg(distinct "parent_documents"."id"::text))'), '=', NULL)
                ->or_having(DB::expr('remove_from_array(array_agg(distinct "parent_documents"."id"::text))'), '=', '{}')
            ->and_having_close()

            ->and_having_open()
                ->or_having(DB::expr('remove_from_array(array_agg(distinct "children_documents"."id"::text))'), '=', NULL)
                ->or_having(DB::expr('remove_from_array(array_agg(distinct "children_documents"."id"::text))'), '=', '{}')
            ->and_having_close()
        ...
        case 'CERT':
          ...
          $ids = array_filter(DB::select('specs_data.id','barcodes.barcode')
            ...
            ->having(DB::expr('coalesce(remove_from_array(array_agg(distinct "barcode_activity"."activity"::text)), \'{}\')'), '&&', DB::expr("array['O', 'S']"))
            ->and_having(DB::expr('NOT coalesce(remove_from_array(array_agg(distinct "barcode_activity"."activity"::text)), \'{}\')'), '&&', DB::expr("array['H', 'Y', 'A', 'Z']"))

            ->and_having_open()
                ->or_having(DB::expr('remove_from_array(array_agg(distinct "cert_documents"."id"::text))'), '=', NULL)
                ->or_having(DB::expr('remove_from_array(array_agg(distinct "cert_documents"."id"::text))'), '=', '{}')
            ->and_having_close()
