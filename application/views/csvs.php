  ...
  <tr id="csv-<?php echo $csv->id; ?>" class="csv-<?php echo $csv->id; ?> <?php print SGS::odd_even($odd); ?>">
   ...
   <?php
    ...
    <td class="<?php echo $hidden_column ? 'hide' : ''; ?> <?php if ($errors[$field] or $errors[SGS::fieldify($field)]): ?>error<?php endif; ?> <?php if ($duplicates[$field] or $duplicates[SGS::fieldify($field)]): ?>duplicate<?php endif; ?>">
      <div id="<?php echo implode('-', array('csv', $csv->id, $field)); ?>" class="<?php if ((Auth::instance()->logged_in('management') or (in_array($csv->form_type, array('SSF', 'SSFV')) and Auth::instance()->get_user()->id == 10) /** FIXME: give albert access to SSF **/ ) and in_array($csv->status, array('P', 'R', 'U'))): ?>csv-eip eip<?php endif; ?>"><?php echo trim($csv->values[$field]); ?></div>
    </td>
    <?php endforeach; ?>
